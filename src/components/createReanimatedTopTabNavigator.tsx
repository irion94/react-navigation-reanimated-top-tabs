...
import {
  ...
  TabBarBaseComponent,
} from './TabBarBaseComponent';
import { Provider } from './TopTabContext';
...
import type { SceneProps } from 'react-native-reanimated-tab-view/src/types/types';
...
const TabViewNavigator = ({
  ...
  const renderScene = useCallback(
    ({ route }: SceneProps) => descriptors[route.key]?.render(),
    []
  );
    ...
    <Provider config={_config} screenProperties={screenProperties}>
      {({
        ...
        <>
          ...
          >
            {HeaderComponent
              ? HeaderComponent({ headerHeight, transformationY })
              : null}
          </Reanimated.View>
          <NavigationContent>
            <GestureWrapper>
